...
> **文档编号**：ENS-LLD-300 ｜ **版本**：V1.4 ｜ **所属架构层级**：L3（数据中枢层）
...
> **修订记录（V1.4）**：Phase 3 切片 9 落地（4.1.6 BlackBoxManager + CriticalSwapFile + PlatformMMap 已实现）。① §3.6.2 `PlatformMMap` 对齐实现：`IMappedFile` 接口 + `createMappedFile()` 工厂（Win32 `CreateFileMapping/MapViewOfFile` + POSIX `mmap/msync`，跨平台二选一编译）。② §3.6.1 `BlackBoxManager::triggerBlackBox` 对齐实现：原子预拷贝（`m_snapshotMutex` 持锁 <10μs + `extractRange` ±30s）→ Critical 级 `CriticalSwapFile::appendSnapshot`（mmap memcpy ~50μs + FlushViewOfFile 异步落盘）。③ §3.6.3 `CriticalSwapRecovery::start` 对齐实现：锁定冲突 → `backup & recreate`。④ **V1.4 简化项（V1.5+ 待补）**：非 Critical 级 JSON 序列化 + L2 异步 persistBlackBox（属切片 10 AlarmEngine 业务层）；mmap 槽位格式 16B/快照固定样本数（实现取 1024：±30s@100ms 含两端点实际 601 样本 > LLD 原文 600 未含端点，V1.5 可改动态长度）；`backup_keep_count` 清理策略未实现（当前保留全部 backup）。  
