...
TEST_CASE("pointtable: resolve(0x1000, 17) returns nullopt (user DoD typo: 17 is PCS range, addr not in PCS BASE)",
    ...
    auto tbl = makeLoadedTable();
    REQUIRE_FALSE(tbl->resolve(/*slave=*/17, /*addr=*/0x1000).has_value());
}

TEST_CASE("pointtable: resolve with non-existent (slave, addr) returns nullopt",
          "[master][pointtable][resolve][neg]") {
    auto tbl = makeLoadedTable();
    REQUIRE_FALSE(tbl->resolve(/*slave=*/99, /*addr=*/0x0000).has_value());
    REQUIRE_FALSE(tbl->resolve(/*slave=*/1,  /*addr=*/0xFFFF).has_value());
}
...
TEST_CASE("pointtable: pointIdOf(out-of-range) returns nullopt",
          "[master][pointtable][pointIdOf][neg]") {
    auto tbl = makeLoadedTable();
    REQUIRE_FALSE(tbl->pointIdOf(0).has_value());
    REQUIRE_FALSE(tbl->pointIdOf(99999).has_value());
}
